...
def main():
    # load model
    model_name = "Track02_model.xgb"
    model = xgboost.Booster()
    model.load_model(model_name)
    
    # collect dtypes
    types = dict(
        zip(SIMPLE_FEATURE_COLUMNS, repeat(np.float32))
    )
    ...
    for chunk in data:
        chunk['prediction'] = model.predict(
            xgboost.DMatrix(chunk.values), ntree_limit=125
        )
        chunk['prediction'].to_csv(
