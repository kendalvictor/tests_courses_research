...
###########español#####################
sw <- readLines("D:/Maestria-URP/2021_IV/Mineria Textos/S3/Osman/stopwordses.txt",encoding="UTF-8")
sw = iconv(sw, to="ASCII//TRANSLIT")

###nuevo documento#####
economia <- readLines("D:/Maestria-URP/2021_IV/Mineria Textos/S3/Osman/economia.txt",encoding="UTF-8")
...
tabes=read.delim("D:/Maestria-URP/2021_IV/Mineria Textos/S3/Osman/lemmatization-es.txt",encoding="UTF-8",header = FALSE, sep = "",
...
economia_tidy=economia_tidy %>%left_join(tabes, by = "term")
economia_tidy$lemma=ifelse(is.na(economia_tidy$stem ), economia_tidy$term, economia_tidy$stem)

#b <- udpipe_annotate(ud_model, x = economia_tidy$texto_tokenizado)
#b <- as.data.frame(b)
names(economia_tidy)[4] <- "texto_tokenizado"
####fecuencia#######
...
wordcloud(words = economia_wc$texto_tokenizado, freq = economia_wc$n,
          min.freq = 4, scale=c(2,.5),random.order = FALSE, rot.per = 0.35,
...
names(economia_tidy)[2] <- "Inicial"
names(economia_tidy)[4] <- "term"
