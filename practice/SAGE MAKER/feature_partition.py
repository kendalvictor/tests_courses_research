...
if __name__=='__main__':
    ...
    cols_target = ['target_desembolso_f2m_mayor_30_menor_180']
    print(cols_target)
    ...
    periodo_train = periodos[:-4]
    periodo_valid = periodos[-4:-2]     ############# CAMBIO AHORA VALIDACION DE DOS MESES
    periodo_seguimiento = periodos[-2:]
    data['periodo'] = data['periodo'].astype(str)
    print(periodos)
    print(periodo_train)
    print(periodo_valid)
    print(periodo_seguimiento)     
    ...
    for normal in [[1], [0, 1]]:                              ## esto deteermina si el entrenamiento será sólo con los 100% normales o todos
        ...
        spearman = train[train['flg_solo_normal'].isin(normal)][cols_target + [col for col in train.columns if col not in cols_id]].corr(method='spearman')
        ...
        for target in cols_target:
            ...
            train[train['flg_solo_normal'].isin(normal)][[target] + [col for col in train.columns if col not in cols_id]].to_csv(
                '{}/train_{}_clasif_{}.csv'.format(path_container_output, target.lower(), clasif), 
                index=False
            )
            validation[validation['flg_solo_normal'].isin(normal)][[target] + [col for col in validation.columns if col not in cols_id]].to_csv(
            ...
            cortes = sc.woebin(
                train[train['flg_solo_normal'].isin(normal)][[target] + [col for col in train.columns if col not in cols_id]],
                y=target,
                method='tree',
                no_cores=16,
                count_distr_limit=0.005
            )
            ...
            seleccion_variables = seleccion_variables.sort_values(
                by=['max_prob'], ascending=False).reset_index(drop=True).reset_index().rename(columns={'index': 'ranking_prob'})
            seleccion_variables = seleccion_variables.sort_values(
                by=['iv'], ascending=False).reset_index(drop=True).reset_index().rename(columns={'index': 'ranking_iv'})
