...
def compare_result(data, codigo_canilla, dicc_predict, order, since_date_1, until_date_1):
    ...
    result['round_ProyHolt'] = result['round_ProyHolt'].apply(
        lambda _: _ + constante_devolucion  + constante_venta
    )
    result['round_HoltWinters'] = result['round_HoltWinters'].apply(
        lambda _: _ + constante_devolucion  + constante_venta
    )
    ...
    result['ahorro_holt'] = result[['diff_int_ventaholt', 'devolucion_real']].apply(
        lambda _: _['devolucion_real'] - calc_indice_ahorro(_['diff_int_ventaholt'], _['devolucion_real'], constante_venta), axis=1
    )
    result['ahorro_new'] = result[['diff_int_ventawinters', 'devolucion_real']].apply(
        lambda _:  _['devolucion_real'] - calc_indice_ahorro(_['diff_int_ventawinters'], _['devolucion_real'], constante_venta), axis=1
    )
