...
def apply_holt_winters(data, range_fold=range(6, 13, 2), range_slen=range(7, 8, 1),
    ...
    if not list_result_select or \
        not all(list_result_select[0][1]) or \
        (first and list_result_select[0][0] > 5):
        
        if not first:
            print("#"*10)
            print("#"*10)
            print("#"*10)
            print("SE TOMARA LA DECISION ENTRE VALORES NO ACEPTABLES")
        else:
            return apply_holt_winters(
                data, 
                range_fold=range(6, 19, 4), 
                range_slen=range(7, 8, 1),
                n_preds=7,
                first=False
            )
